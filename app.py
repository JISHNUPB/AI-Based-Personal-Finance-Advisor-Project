...
# -------------------------------------------
# 📤 Input Options (CSV Upload + Manual Input)
# -------------------------------------------
st.markdown("### 📂 Choose Your Input Method")

# Create tabs for different input methods
tab1, tab2 = st.tabs(["📁 Upload CSV File", "⌨️ Type/Paste SMS"])

df = None  # Initialize df variable

with tab1:
    st.markdown("#### Upload your SMS transaction file")
    uploaded_file = st.file_uploader("Choose a CSV file (must contain 'SMS' column)", type=["csv"], key="csv_uploader")
...
with tab2:
    st.markdown("#### Type or paste your SMS messages")
    st.markdown("Enter one SMS per line. Example:")
    st.code("""Your a/c XX1234 debited with ₹500 at SWIGGY
Credit of ₹20000 from SALARY received
Payment of ₹1500 to AMAZON""")
    
    manual_input = st.text_area(
        "Enter SMS messages (one per line):",
        height=150,
        placeholder="Your a/c XX1234 debited with ₹500 at SWIGGY\nCredit of ₹20000 from SALARY received\nPayment of ₹1500 to AMAZON",
        key="manual_input"
    )
    
    if st.button("Analyze Text Input", key="analyze_manual"):
        if manual_input.strip():
            # Convert manual input to DataFrame
            sms_list = [line.strip() for line in manual_input.split('\n') if line.strip()]
            df = pd.DataFrame({'SMS': sms_list})
            st.success(f"✅ {len(sms_list)} SMS messages loaded for analysis!")
        else:
            st.warning("⚠️ Please enter at least one SMS message.")
...
if df is not None:
    ...
    top_cat = df.groupby("Predicted_Category")["Amount"].sum().idxmax() if not df.empty else "N/A"
